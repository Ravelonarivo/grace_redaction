...
const Blog = ({ blogSectionRef }) => {
	...
	return (
		<div ref={ blogSectionRef } id="blog" className={ styles.background + ' relative mt6 ph7 pb4' }>
			<div className={ styles.square + ' absolute' }></div>
			<div className={ styles.circle + ' absolute br-100' }></div>
			<div className={ styles.square_1 + ' absolute br4' }></div>
			<div className={ styles.circle_1 + ' absolute br-100' }></div>
			<h2 ref={ titleRef } className={ styles.title + ' tc f2 dark-blue fw5 animate__animated' }>Consultez notre blog</h2>
			{
				articles.map((article, index) => (
					<div ref={ element => articleRefs.current[index] = element  } key={ index }  className={ styles.articleVisibility + ' dib pointer grow h5 w-100 mb4 ba b--black-10 pa3 shadow-4 br4 bg-white animate__animated' }>
